%% RMS_metric
% Compute the root means square of the difference between the vectors of the two deformation fields
%
%% Syntax
% |res = RMS_metric(im1,im2,handles)|
%
%
%% Description
% |res = RMS_metric(im1,im2,handles)| Compute the root means square of the difference between the vectors of the two deformation fields
%
%
%% Input arguments
...
%% Output arguments
%
% |res| - _SCALAR_ - Root means square of the difference between the vectors of the two deformation fields
%
%% Contributors
% Authors : G.Janssens (open.reggui@gmail.com)

function res = RMS_metric(im1,im2,handles)
...
try
    square_diff = squeeze(sum((myIm1-myIm2).^2,1))/3;
    if(use_roi_mode)        
        res = sqrt( sum(sum(sum( square_diff.*handles.images.data{use_roi_mode} ))) /sum(sum(sum(handles.images.data{use_roi_mode}))) );
        disp(['Root mean square (on ROI) = ' num2str(res)])
    else
        res = sqrt( sum(sum(sum( square_diff ))) /size(myIm1,2)/size(myIm1,3)/size(myIm1,4) );
        disp(['Root mean square = ' num2str(res)]);
    end
catch ME

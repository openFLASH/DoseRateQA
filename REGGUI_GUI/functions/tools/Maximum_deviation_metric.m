...
% Compute the maximum difference between the vectors of the two deformation fields
%
%% Syntax
% |res = Maximum_deviation_metric(im1,im2,handles)|
%
%
%% Description
% |res = Maximum_deviation_metric(im1,im2,handles)| Compute the maximum difference between the vectors of the two deformation fields
%
%
%% Input arguments
...
%% Output arguments
%
% |res| - _SCALAR_ - Maximum difference between the vectors of the two deformation fields
%
%% Contributors
% Authors : G.Janssens (open.reggui@gmail.com)

function res = Maximum_deviation_metric(im1,im2,handles)
...
try
    if(use_roi_mode)
        ...
        res = max(reshape(m,1,n*length(m)));
        disp(['Maximum deviation (in ROI) = ' num2str(res)])
    else
        for n=1:3
            m(n,:) = reshape(squeeze(abs(myIm1(n,:,:,:)-myIm2(n,:,:,:))),1,size(myIm1,2)*size(myIm1,3)*size(myIm1,4)); 
        end
        res = max(reshape(m,1,n*length(m)));
        disp(['Maximum deviation = ' num2str(res)]);
    end
catch ME

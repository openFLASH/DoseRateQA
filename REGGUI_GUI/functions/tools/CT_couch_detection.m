%% CT_couch_detection
% Identify the couch in the CT image. Returns a mask defining the position of the couch.
%
%% Syntax
% |res = CT_couch_detection(handles,image_name,couch_name)|
%
%
%% Description
% |res = CT_couch_detection(handles,image_name,couch_name)| describes the function
%
%
%% Input arguments
...
%
% |image_name| - _STRING_ -  Name of the CT in |handles.images| in which the couch should be identified
%
...
function handles = CT_couch_detection(handles,image_name,couch_name)
...
intensities = Get_reggui_data(handles,image_name);
...
handles = Opening(couch_name,[1 3 1],'temp_CT_seg_1',handles);
handles = Difference(couch_name,'temp_CT_seg_1','temp_CT_seg_2',handles);
handles = Opening('temp_CT_seg_2',[3 1 1],'temp_CT_seg_3',handles);
transitions = sum(sum(handles.images.data{end},3),1);
transitions(transitions<10*mean(transitions)) = 0;
transitions(intensities<0.8)=0;

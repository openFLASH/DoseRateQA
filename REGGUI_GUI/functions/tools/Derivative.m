%% Derivative
% Apply the partial derivative filter (FD, DerivativeImageFilter) from the Matitk library [http://matitk.cs.sfu.ca/search] on an image
%
%% Syntax
% |handles = Derivative(image,params,im_dest,handles)|
%
%
%% Description
% |handles = Derivative(image,params,im_dest,handles)| Compute partieal derivative of an image
%
%
%% Input arguments
% |image| - _STRING_ -  Name of the image contained in |handles.images.name| to be processed 
%
% |params| - _INTEGER VECTOR_ -  Parameters for the ITK function. |params= [order,direction]|
%
...
function handles = Derivative(image,params,im_dest,handles)
...
if(ndims(myImage)==2)
    ...
    im_res = matitk('FD',params,myImage);

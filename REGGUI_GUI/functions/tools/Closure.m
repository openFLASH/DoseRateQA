%% Closure
% Apply a closure filter on an image. the function applies an |imdilate| filter followed by |imerode|.
%
%% Syntax
% |handles = Closure(image,params=[x,y,z],im_dest,handles)|
%
% |handles = Closure(image,params=[i,j],im_dest,handles)|
%
% |handles = Closure(image,params,im_dest,handles,shape)|
%
% |handles = Closure(image,params,im_dest,handles,shape)|
%
%
%% Description
% |handles = Closure(image,params,im_dest,handles)| Apply a closure filter on the image using a spheric kernel element
%
% |handles = Closure(image,params,im_dest,handles,shape)| Apply a closure filter on the image
%
%
%% Input arguments
...
function handles = Closure(image,params,im_dest,handles,shape)
...
im_res = imdilate(single(myImage),myStrel);

%% regularisation_gauss_discontinuous
% Performs a discountinuous gaussian smoothing of the deformation/displacement field.
% The discountinuous smoothing is described in section "4.4 Sliding surfaces" of reference [1]
%
%% Syntax
% |[out_field, out_cert] = regularisation_gauss_discontinuous(proc, in_field, in_cert, scale, data)|
%
%
%% Description
% |[out_field, out_cert] = regularisation_gauss_discontinuous(proc, in_field, in_cert, scale, data)| Performs gaussian smoothing of the deformation/displacement field
%
%
%% Input arguments
%
...
function [out_field, out_cert] = regularisation_gauss_discontinuous(proc, in_field, in_cert, scale, data)
% function [out_field, out_cert] =
% regularisation_gauss_discontinuous(proc, in_field, in_cert, scale)
...
if(ndims(input.rescaled_mask) > 1)
    ...
    out_cert = gauss_smoothing(in_cert, sigma_mod);
    
    in_cert = ones(size(in_cert),'single');
    ...
    for n = 1:dims
        out_field{n} = gauss_smoothing(fn{n}, sigma_mod);
    end
...
%     end
    ...
    for n = 1:dims
        out_field{n} = gauss_smoothing(in_field{n}, sigma_mod);
    end
    out_cert = gauss_smoothing(in_cert, sigma_mod);

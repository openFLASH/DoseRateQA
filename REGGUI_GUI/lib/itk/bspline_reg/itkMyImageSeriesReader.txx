/*=========================================================================
  ...
  Module:    $RCSfile: itkMyImageSeriesReader.txx,v $
  Language:  C++
  Date:      $Date: 2006/02/02 18:26:53 $
...
=========================================================================*/
#ifndef _itkMyImageSeriesReader_txx
#define _itkMyImageSeriesReader_txx
#include "itkMyImageSeriesReader.h"
...
MyImageSeriesReader<TOutputImage>
::~MyImageSeriesReader()
{
...
template <class TOutputImage>
void MyImageSeriesReader<TOutputImage>
...
::GenerateOutputInformation(void)
{
  ...
  typedef MetaDataObject<std::string> MetaDataDictStringType;
  ...
  if (m_FileNames.size() > 1)
    ...
    try
      ...
      if (interSliceSpacing == 0.0)	
        {
	std::string tagvalue = "1.0";
/*
	const  DictionaryType & dictionary = reader1->GetImageIO()->GetMetaDataDictionary();
	std::string spacingBetweenSlicesID = "0018|0088";
	DictionaryType::ConstIterator end = dictionary.End();
	DictionaryType::ConstIterator tagItr = dictionary.Find( spacingBetweenSlicesID );
	if( tagItr == end )
      		{
      		std::cerr << "Dicom tag 'spacingBetweenSlice' (" << spacingBetweenSlicesID;
      		std::cerr << ") not found in the header. Setting z-spacing to default (1)." << std::endl << std::endl;
		tagvalue = "1.0";
      		}
	else
        MetaDataDictStringType::ConstPointer entryvalue = 
        dynamic_cast<const MetaDataDictStringType *>( tagItr->second.GetPointer() );
    	if( entryvalue )
      		{
...
*/
	char * pEnd;
	interSliceSpacing = strtod(tagvalue.c_str(),&pEnd);;
	}        
      }
...
template <class TOutputImage>
typename 
MyImageSeriesReader<TOutputImage>::DictionaryArrayRawPointer 
MyImageSeriesReader<TOutputImage>

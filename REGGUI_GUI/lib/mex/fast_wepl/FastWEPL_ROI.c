...
/*
compile with matlab (linux): mex CFLAGS='\$CFLAGS -fopenmp' LDFLAGS='\$LDFLAGS -fopenmp' FastWEPL_ROI.c
compile with matlab (windows): mex COMPFLAGS='$COMPFLAGS /openmp' LINKFLAGS='$LINKFLAGS /openmp' FastWEPL_ROI.c
run with matlab: WEPL = FastWEPL_ROI(WET, BeamAngle, Spacing, ROI);
*/
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
    ...
    if (nrhs != 4) mexErrMsgTxt("This function takes 4 arguments: 3D WET map, beam angle, voxel spacing, ROI");
    ...
    /* Fourth input (prhs[3]) is binary mask of the ROI */
    if(!mxIsNumeric(prhs[3]) || !mxIsSingle(prhs[3]) || mxIsSparse(prhs[3]) || mxIsComplex(prhs[3]) || mxIsCell(prhs[3])){
        mexErrMsgTxt("Fourth argument (ROI) must be full numeric array of real numbers, stored as singles.");
    }
    if(mxGetNumberOfDimensions(prhs[3]) != 3){
        mexErrMsgTxt("Fourth argument (ROI) must have 3 dimensions.");
    }
    float *mask = (float*)mxGetPr(prhs[3]);
    ...
    for(i=0; i<GridSize[0]; i++){
		...
		if(mask[offset_WEPL] == 0) continue;

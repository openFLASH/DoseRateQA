%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% MatLab Utility for exploring a treatment plan and plotting their fields
% Syntax: explore_plan(plan)
%
% Modified for RT plan exploration
% Modified from CRC - ExploreStruct v1.2, version R2012a by Luke Plausin 2015
...
function explore_plan(varargin)
...
txt4 = uicontrol('String', '', 'Units', 'normalized', 'Style', 'Text',...
    'Position', [.36 .005 .72 .1], 'BackgroundColor',  [.925 .914 .847],...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function plotselected_cb(obj, ev, h)
        ...
        plotted = strrep(plotted,'_','\_');
        
        if(isstruct(val))
            if(isfield(val,'energy') && isfield(val,'xy') && isfield(val,'weight')) % spot map
                for layer=1:length(val)
                    scatter(haxes,val(layer).xy(:,1),val(layer).xy(:,2),min(val(layer).weight(:),10)*300+15);
                    hold on
                end
                xlabel('x')
                ylabel('y')
                hold off
                title(haxes,plotted)
            elseif(isfield(val,'spots') && length(val)==1) % beam
                for layer=1:length(val.spots)
                    errorbar(haxes,layer,val.spots(layer).energy,sum(val.spots(layer).weight),'o');
                    hold on
                end
                xlabel('Layer sequence [Bar = total MUs]')
                ylabel('Energy')
                hold off
                try
                    title(haxes,val.name)
                catch
                end
            elseif(isfield(val,'beams') ) % beamset
                angle_colors = [(0:359)',ones(360,1),(359:-1:0)']/360;
                for b=1:length(val.beams)
                    errorbar(haxes,b,val.beams(b).gantry_angle,length(val.beams(b).spots),'Color',angle_colors(mod(val.beams(b).table_angle,360)+1,:));
                    hold on
                end
                xlabel('Beam sequence')
                ylabel('Gantry angle [Bar = nb of layers]')
                hold off
            end
        elseif isnumeric(val) && length(val)>1
            ...
            disp('variable to be plotted should be numeric with length > 1')
        end
    ...
    function cNode = nodeSelected_cb4(tree,ev,h)
        ...
        if(isstruct(val))
            if(isfield(val,'beams'))
                ...
                max_E = -Inf;
                for n=1:length(p)
                    nb_layers = nb_layers + length(p(n).spots);
                    if(isfield(p(n),'final_weight'))
                        total_mu = total_mu + p(n).final_weight;
                    else
                        total_mu = NaN;
                    end
                    for i=1:length(p(n).spots)
                        nb_spots = nb_spots + length(p(n).spots(i).weight);
                        min_E = min(min_E,p(n).spots(i).energy);
                        max_E = max(max_E,p(n).spots(i).energy);
                    end
                end
                val = cell(0);
                val{end+1} = ['Number of beams : ',num2str(nb_beams),';   Number of layers : ',num2str(nb_layers),';   Number of spots : ',num2str(nb_spots)];
                val{end+1} = ['Total MUs : ',num2str(total_mu)];
                val{end+1} = ['Min-max energy : ',num2str(min_E),' - ',num2str(max_E)];
            end
        end
        try
            set(txt4, 'string', val)

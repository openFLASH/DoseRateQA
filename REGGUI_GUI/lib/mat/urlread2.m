...
% * Allows specification of any HTTP method
% *  Allows specification of any header. Very little is hard-coded in for header handling.
...
%
%% Description
% |[output,extras] = urlread2(urlChar, *method, *body, *headersIn, varargin)| Description
%
...
%
% |headersIn| - _STRUCTURE ARRAY_ - use empty [] or '' if no headers are needed
%                   but varargin property/value pairs are, multiple headers
%                   may be passed in as a structure array
% * |headersIn.name| - _STRING_ - name of the header, a name property is used
%                   instead of a field because the name must match a valid
%                   header
% * |headersIn.value| - _STRING_ - value to use
%
...
% |extras| - _STRUCTURE_ -
%
% * |extras.allHeaders| - _STRUCTURE_ - fields have cellstr values, HTTP headers may 
...
% * |extras.firstHeaders| - _STRUCTURE_ - variable fields, contains the first
...
%                   EXAMPLE:
% * ---|extras.firstHeaders.Response| = 'HTTP/1.1 200 OK'}
% * ---|extras.firstHeaders.Server| = 'nginx'
% * ---|extras.firstHeaders.Date| = 'Tue, 29 Nov 2011 02:23:16 GMT'
% * ---|extras.firstHeaders.Content_Type| = 'text/html; charset=UTF-8'
% * ---|extras.firstHeaders.Content_Length| = '109155'
% * ---|extras.firstHeaders.Connection| = 'keep-alive'
% * ---|extras.firstHeaders.Vary| = 'Accept-Encoding, User-Agent'
% * ---|extras.firstHeaders.Cache_Control| = 'max-age=60, private'
% * ---|extras.firstHeaders.Set_Cookie| = 'first_value'
% * |extras.status| - _STRUCTURE_ - 
% * ---|extras.status.value|  : numeric value of status, ex. 200
% * ---|extras.status.msg|    : message that goes along with status, ex. 'OK'
% * |extras.url| - _STRING_ -eventual url that led to output, this can change from
%                     the input with redirects, see FOLLOW_REDIRECTS
% * |extras.isGood| - _LOGICAL_ - I believe this is an indicator of the presence of 400
...
%
%% Contributors
% Authors : Jim Hokanson
%   Copyright (c) 2012, Jim Hokanson
%   All rights reserved.

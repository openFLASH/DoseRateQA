...
function [records,outputDir,sum_MU , beaminfo] = load_IBA_logs(logFilename, outputDir, xdr_converter, merge_tuning, remove_negative_spots)
...
for i=1:1:size(logFiles,1)
    ...
    if ~(isempty(strfind(logFiles(i).name,'beam.')))
        ...
        [mBeamId,mGantryAngle , beaminfo] = get_beam_info_from_log_beamfile(beam_filename);
        records{1}.BeamName = mBeamId;
        mGantryAngle = str2double(mGantryAngle);
    end

    if ~(isempty(strfind(logFiles(i).name,'events.')))
        beam_filename = fullfile(outputDir,logFiles(i).name);
        disp(['Processing beam file: ' beam_filename]);
        eventinfo = get_beamEvent_info(beam_filename);
        beaminfo=mergestruct(beaminfo,eventinfo);
    end
end

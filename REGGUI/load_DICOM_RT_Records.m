...
function [myBeamData,myInfo, beaminfo] = load_DICOM_RT_Records(log_filename,format,ref_data,ref_info,varargin)
...
switch format
    ...
    case 'iba' % IBA LOG FILES ------------------------------------------
        ...
        for f=1:nb_logs
            ...
            % Load logs
            [C,~,final_weight, beaminfo] = load_IBA_logs(log_filename{f},'',XDRconverter,merge_tuning);

%% reg_animate2D
%
% This is the function carrying out the actual computation of the multi-scale non-rigid deformation for 2D images. See reference [1] for more information on the registration process.
% 
...
% * ----|reg.process(p).input| - _STRUCTURE_ - Structure describing the fixed 2D image for process |p|. See description in 'reg_input.m'. 
% * ----|reg.process(p).input.cert| - _MATRICE_ - Certainty about the value of the intensity at voxel (x,y) of the input data. In some applications, the accuracy of intensity values may be reduced for some voxels due to acquisition noise or image processing singularities. In some cases, the pixel values are even a biased representation of some hidden truth. When the certainty relative of each pixel value is known, this information can be taken into account by a filtering process using normalized convolution. See reference [1], section 2.1.3 Image Filtering (b) Normalized convolution.
% * ----|reg.process(p).prototype| - _STRUCTURE_ - Structure describing the mobile 2D image for process |p|. See description in 'reg_prototype.m'
...
% * ----|reg.process(p).q_in{n}| - _SCALAR MATRIX_ reg.process(p).q_in{n}(x,y) Intensity of the voxel (x,y) of the rescaled input image |input.rescaled_data| convolved with the quadrature filter kernel |n| for process |p|
%
...
%
% * |reg.sz| - _VECTOR of SCALAR_ Dimension (x,y) (in pixels) of the fixed image. Length is equal to |reg.dims|.
%
% * |reg.spacing| - _VECTOR of SCALAR_ - Size (x,y) (in |mm|) of the pixels in the images.
%
...
% * ----- |reg.live.accumulated_deformation_field| - _CELL VECTOR of MATRICES_ |accumulated_deformation_field{1}(x,y)| is X component of the deformation field at the voxel (x,y). The Y component is defined in cell components {2}. The number of cell  elements is equal to |reg.dims|.
% * ----- |reg.live.accumulated_deformation_certainty| _MATRICE_ Certainty about the value of the deformation field at voxel (x,y) for the current iteration. See reference [1], section 2.1.3 Image Filtering (b) Normalized convolution.
%
%% Output arguments
%
...
function iterations_per_scale = reg_animate2D()
...
while(scale >= reg.resampling.min_scale)
    ...
    for p=1:reg.nb_process
        ...
        if(sum(size(reg.process(1).input.mask))==sum(size(reg.process(1).input.data)==2))
            disp(reg.process(1).input.mask)
            ...
            if(sum(sum(reg.process(p).input.rescaled_dm)))
                [gx gy] = gradient(reg.process(p).input.rescaled_dm);
                reg.process(p).input.rescaled_grad{1} = gx./(sqrt(gx.^2+gy.^2)+eps);
                reg.process(p).input.rescaled_grad{2} = gy./(sqrt(gx.^2+gy.^2)+eps);
                clear gx gy;
            else
                disp('Mask not valid!');
            end
    ...
    for iter = 1:2*iters
        ...
        % Accumulate Field
        if(ndims(reg.process(1).input.rescaled_mask)==2)            
        ...
        if(use_mnorm_metric)
            disp(['Iteration ', num2str(iter), ' of ', num2str(2*iters), '  ------------------------->   SSD : ', num2str(metric_SSD),'   and   field MNorm : ', num2str(floor(metric_MNorm*1000))])
        elseif(use_cc_metric)
            disp(['Iteration ', num2str(iter), ' of ', num2str(2*iters), '  ------------------------->   SSD : ', num2str(metric_SSD),'   and   CC : ', num2str(metric_CC)])
        else
            disp(['Iteration ', num2str(iter), ' of ', num2str(2*iters), '  ------------------------->   SSD : ', num2str(metric_SSD)])
        end
...
subplot(2,2,1)
a = reg.process(1).input.rescaled_data;
b = reg.process(1).deformed_prototype.data;
c = reg.process(1).prototype.rescaled.data;
im = [(a-c),c,a;(a-b),b,a];
...
quiver((reg.live.accumulated_deformation_field{2}.*subsample)', (reg.live.accumulated_deformation_field{1}.*subsample)',0,'r');
axis image
axis xy
axis off
...
if(reg.nb_process>1)
    subplot(2,2,3)
    a = reg.process(2).input.rescaled_data;
    b = reg.process(2).deformed_prototype.data;
    c = reg.process(2).prototype.rescaled.data;
    im = [(a-c),c,a;(a-b),b,a];
    imagesc(im'), colormap gray
    title('Before deformation     -     After deformation')
    axis image
    ...
    if(metric_CC_tot)
        ...
        imagesc(reg.live.accumulated_deformation_certainty'), colormap gray
        title('Certainty')
        axis image
        axis xy
        axis off

    ...
    for optFidx = 1:length(Plan.optFunction)
        ...
        if (Plan.optFunction(optFidx).ID == 8 || Plan.optFunction(optFidx).ID == 9)
            ...
            if ~isfield(Plan.optFunction(optFidx),'Vref')
              ...
              percentile = 0.025; %Default percentile for dose rate
            else
            ...
            for b = 1:length(DRaStru)
              ...
              [handles, doseRateName] = save2Disk(handles, DRaStru{b}, Plan.DoseGrid.size, Plan.CTinfo, ['PDR95_beam_',num2str(b),'_in_' , Plan.optFunction(optFidx).ROIname], path2beamResults , planFullPath ,'PERCENTILE');

...
function [DoseRate , DRa , DRmin, DRmax , DRm , Tstart , Tend , DADR , DADRm, DRAD, DRADm, SpotTiming , pxlSelected , MPDR, DRhisto ] = DRaEstimate(spotSequence , dT , TimePerSpot , Dose , DMF, DR50, DoseTimingStruct , percentile, Nmaps)
  ...
  if nargin < 6
    ...
    if nargin < 7
      ...
      fig = DoseTimingStruct.fig;
      FileName = DoseTimingStruct.FileName;
    end
    ...
    if isempty(percentile)
      percentile = 0.025; %Compute dose rate for the 95-percentile
    end
    if (percentile < 0) || (percentile > 1)
      percentile = 0.025; %Default value of percentile if an invalid value is given
    end
    ...
    if ~isempty(fig)
      ...
      if ~isempty(FileName)
        %Save all data to compute spot timing curve later on
        %DoseTimingStruct.pxlSelected = pxlSelected;
        DoseTimingStruct.spotTimingStart = spotTimingStart;
        DoseTimingStruct.Dose = Dose(spotSequence , :);
        DoseTimingStruct.TimePerSpot = TimePerSpot;
        fprintf('Saving spot timing information ...')
        save(FileName , 'DoseTimingStruct','-v7.3')
        fprintf('Done \n')
      end
    else
      pxlSelected = [];
    ...
    MPDR = 0; %Computing MPDR is too slow. We skip it and set a default value.

    fprintf("Starts the DADR computation \n");
    tic
    % Compute dose averaged dose rate
    DADR = doseAveragedDoseRate(Dose(spotSequence,:) , spotTimingStart, spotTimingStop);
    toc
    fprintf("Ended the DADR computation \n");

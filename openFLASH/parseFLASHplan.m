...
function [handles, Plan] = parseFLASHplan(planFileName , Plan, handles)
    ...
    setFlashDICOMdict(Plan.DICOMdict); %If not already defined, load the DICOM dictionary with private FLASH tags
    ...
    for b = 1:NbBeams
        ...
        Plan.MachineNameDICOM = monoPlan.IonBeamSequence.(itemBeam).TreatmentMachineName; %Machine name from DICOM treatment plan

        fprintf('BDL name      : %s \n' , Plan.BDL);
        [Plan.MachineType , Plan.Machine.name] = getMachineFromBDL(Plan.BDL); %Read machine type and machine name from BDL
        fprintf('Machine name in BDL         : %s \n' , Plan.Machine.name)
        fprintf('Machine type in BDL         : %s \n' , Plan.MachineType)
        fprintf('Machine name in DICOM plan  : %s \n' , Plan.MachineNameDICOM)

        if ~strcmp(Plan.MachineNameDICOM , Plan.Machine.name)
          warning('Mismatch in machine name between plan and BDL')
        end
        ...
        if Plan.Beams(b).NumberOfRangeShifters
              ...
              if ~isfield(monoPlan.IonBeamSequence.(itemBeam).IonControlPointSequence.Item_1.RangeShifterSettingsSequence.Item_1, 'RangeShifterWaterEquivalentThickness')
                ...
                [~, ~, SPRrs] =  getMaterialPropCT(Plan.Beams(b).RSinfo.RangeShifterMaterial, Plan.ScannerDirectory , fullfile(Plan.MCsqExecPath , 'Materials')); %CEM relative stopping power

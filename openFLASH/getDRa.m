...
for b = 1:length(sobp) %Loop for each beam
    ...
    if (NbPixels)
        ...
        for beamletIndex = 1:NbBeamlets
          ...
          %NB : There is a strange behaviour with the .* multiplication of sparse matrix.
        ...
        DoseTimingStruct.fig = plotID.plot_DR;
        DoseTimingStruct.FileName = fullfile(OutputDir,'DoseTimingData.mat');
        [tmp , dr(b) , DRmin, DRmax, drm(b), Tstart , Tend , DADRtmp , DADRm(b), ~, ~, SpotTiming{b}, pxlSelected] = DRaEstimate(sobpSequence , dT , TimePerSpot , Dose , DMF, DR50, DoseTimingStruct , percentile , SpotTrajectoryInfo.beam{b}.Nmaps); %Average dose rate at several measurement point (MP) in ROI. The MP are located at the centre of the spot. Same order than |weight|
        DRhisto = [];
        ...
        k = Plan.DoseGrid.size(3)-k+1; %Flip the 3rd dimension
        ...
        fprintf('Maximum dose at [%f , %f , %f ]mm (IECg) \n',Pgantry(1),Pgantry(2),Pgantry(3))

        FileName = fullfile(OutputDir,'DoseTimingConfig.mat');
        DoseTimingConfig.T = T;
        fprintf('Saving config information for timing ...')
        save(FileName , 'DoseTimingConfig','-v7.3')
        fprintf('Done \n')

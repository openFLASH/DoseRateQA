...
function plotDoseRate(spotTimingStart , Dose , TimePerSpot , figNb)
  ...
  DADR2 = doseAveragedDoseRate(Dose , spotTimingStart, spotTimingStart+TimePerSpot); %Compute DADR from time trace. Must be equal to DADR from sdose rate map

  figure(figNb)

  yyaxis left
  plot(timeAxis, DoseRatePoint)
  text(50,50,['<DADR> = ' num2str(DADR2) 'Gy/s'])

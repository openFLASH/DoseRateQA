...
function [Plan , MinDose , MaxDose , DoseOrig] = computeDoseWithCEF(Plan, outputPath, handles, CTName , FLAGdosePerSpot)
    ...
    % CAUTION : The density of the material is lineraly interpolated. This is why in HU_Density_Conversion.txt, the density of the material is added to the abscisse HU+1 compared to 'HU_Material_Conversion.txt'
...
function [DoseOrigCT, DoseFileName , handlesDose , Pij] = getFullHighResDosemap(Plan , handles , Zdistal, outputPath , CTName , PijFlag, hCT)
  ...
  % Set coarse grid for dose calculation.
  %The dose map is aligned with the origin of the CEM to avoid alisaing problems
  [Plan , iDoseGntX , iDoseGntY , iDoseGntZ , minField , maxField , hDoseIECg] = getHRdoseGridInfo(Plan , handles, Zdistal);
  ...
  for spt = 1:NbBeamlets
      ...
      [handles, doseSpt, DoseFileName , hDoseSpt] = getHighResDose(Plan, outputPath, handles, CTName, spt, minField , maxField , Zdistal , false , iDoseGntX , iDoseGntY , iDoseGntZ); %Add the dose of each spot to the global high resolution dose map

      DoseIECg = DoseIECg + doseSpt; %Ass all spot map have the same size, we can just add them up
...
function [DoseDCMcs, handlesDose] = doseIECg2DICOMcs(DoseIECg , handlesDoseIECg , hD , Plan , outputPath)
  ...
  %
  % OUTPUT
  % |handles| -_STRUCTURE_- REggui data handle. Added here to keep the handlesHR structure for every beamlet
  %
  ...
  function [handles, Dose, DoseFileName , DoseHR] = getHighResDose(Plan, outputPath , handles , CTName , spt , minField , maxField , Zdistal , WholeField , iDoseGntX , iDoseGntY , iDoseGntZ)
    ...
    if ~isfield(handles, 'handlesHR')
       %Generate the high resolution CT scan
        ...
        PlanHR  = updatePlanCTparam(handles.handlesHR , PlanHR );
        ...
        handles.handlesHR = setRangeShifterinHRCT(handles.handlesHR , PlanHR , hrCTName);
        ...
        %if (~exist(DoseFullFileName) && Plan.SaveHighResCT)
          ...
          CTintrp = Get_reggui_data(handles.handlesHR, hrCTName ,'images'); %Get the high res CT
          infoCTinterp = PlanHR.CTinfo;
          infoCTinterp.ImagePositionPatient = handles.handlesHR.origin;
          infoCTinterp.Spacing = handles.handlesHR.spacing;
    ...
    PlanMono = saveMonoLayerPlan(Plan , spt ,  handles.handlesHR.isocenter , handles.handlesHR.origin , handles.handlesHR.spacing , WholeField  , outputPath);
    ...
    Plan2.Scoring_origin = [iDoseGntX(1) , -iDoseGntZ(end) , iDoseGntY(1)]; %The first pixel of the spot dose map is algined with the pixels of the global dose map

    %The dose scoring map is the same as the global dose map
    Plan2.Scoring_grid_size =  [numel(iDoseGntX) , numel(iDoseGntZ) , numel(iDoseGntY)];
    ...
    if (~exist(DoseFullFileName))
      ...
      if Plan.SaveHighResDoseMap
        ...
        handles.handlesHR = ComputeFinalDose(Plan2, handles.handlesHR , DoseFileName);
      else
        %The user does not want to save the high resolution dose map in IEC gantry
        handles.handlesHR = ComputeFinalDose(Plan2, handles.handlesHR , []);
      end

      [Dose , DoseInfo ] = Get_reggui_data(handles.handlesHR,'dose_final_miropt');
                      %Get the image from handles.handlesHR.myData. This is the idx-th element with |handles.handlesHR.myData.name{idx} = 'dose_final_miropt'|
      ...
      handles.handlesHR = Remove_data('dose_final_miropt', handles.handlesHR); %Delete the dose map so that next dose map keep same name
    ...
    DoseHR.isocenter = [handles.handlesHR.isocenter(1) , handles.handlesHR.isocenter(3) , handles.handlesHR.isocenter(2)]; %permuted the CT scan
...
function PlanMono = saveMonoLayerPlan(Plan , spt , isocenterINTER , ImagePositionPatientINTER , spacingINTER , WholeField , outputPath)
  ...
  createDICOMPlan(PlanMono,PlanMono.CTinfo,fullfile(outputPath,'CEF_beam'),Plan.DICOMdict)
  ...
  if isfield( PlanHR.Beams , 'RSinfo')
      ...
      for slab =  PlanHR.Beams.RSinfo.NbSlabs : -1 : 1
          ...
          ZgUp =  PlanHR.Beams.RSinfo.IsocenterToRangeShifterDistance +  ... %Donwstream side of slab close to isocenter
                 ...
                 handlesHR.spacing(2) ./2 ; %Remove half a pixel to getthe coordinate of the center of last pixel. The Zg axis is the second index of the matrix

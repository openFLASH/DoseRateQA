...
function Plan = overwriteWithLogs(Plan, spots)
  ...
  if abs(spots.gantry_angle - Plan.Beams(idxBeam).GantryAngle) > 0.2
    fprintf('Gantry angle in plan : %f ° \n', Plan.Beams(idxBeam).GantryAngle )
    fprintf('Gantry angle in logs : %f ° \n', spots.gantry_angle )
    warning('Gantry angle mismatch between log and plan')
  end

  if ~strcmp(Plan.MachineNameDICOM , spots.beamInfoInLogs.mBeamDeliveryPointId)
    fprintf('Machine name in logs        : %s \n' , spots.beamInfoInLogs.mBeamDeliveryPointId)
    fprintf('Machine name in DICOM plan  : %s \n' , Plan.MachineNameDICOM)
    warning('Mismatch in machine name between plan and logs')
  end
